...
public class VacancyControllerTest {
    ...
    public void whenPostVacancyWithFileThenSameDataAndRedirectToVacanciesPage() throws Exception {
        ...
        doNothing().when(vacancyService).save(vacancyArgumentCaptor.capture(), fileDtoArgumentCaptor.capture());
    ...
    public void whenSomeExceptionThrownThenGetErrorPageWithMessage() {
        ...
        doThrow(expectedException).when(vacancyService).save(any(), any());
    ...
    public void whenRequestNonExistentVacancyByIdThenGetErrorPage() {
        when(vacancyService.findById(999)).thenReturn(Optional.empty());

        var model = new ConcurrentModel();
        var view = vacancyController.getById(model, 999);
    ...
    @Test
    public void whenUpdateVacancyThenSameDataAndRedirectToVacanciesPage() throws Exception {
        var vacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var vacancyArgumentCaptor = ArgumentCaptor.forClass(Vacancy.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
    ...
    @Test
    public void whenUpdateNonExistentVacancyThenGetErrorPage() {
    ...
    @Test
    public void whenUpdateVacancyWithExceptionThenGetErrorPage() {
        var vacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
        var expectedException = new RuntimeException("Update failed");
        doThrow(expectedException).when(vacancyService).update(any(Vacancy.class), any(FileDto.class));
    ...
    public void whenDeleteNonExistentVacancyThenGetErrorPage() {
        when(vacancyService.deleteById(999)).thenReturn(false);

        var model = new ConcurrentModel();
        var view = vacancyController.delete(model, 999);
        var actualMessage = model.getAttribute("message");
